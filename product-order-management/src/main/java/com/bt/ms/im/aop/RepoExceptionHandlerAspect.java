	...
	public Object catchExceptionAroundRepositoryCommon(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			log.error("Exception cached in repo aspect :: occurred on :: {} :: Arguments :: {} Exception Details :: {} {}",
					joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()), ex,
		...
		/*
		 * required if soap call are there catch (SoapMessageCreationException |
		 * WebServiceIOException ex) { BptmHelper.logBPTMError(ex); log.
		 * error("Exception cached in repo aspect :: occurred on :: {} :: Arguments :: {} Exception Details :: {} {}"
		 * , joinPoint.getSignature().getName(), Arrays.toString(joinPoint.getArgs()),
		 * ex.toString(), Arrays.toString(ex.getStackTrace())); return
		 * ResponseBean.errorRes(this.appConstants.getErrorRes().getClientErrorCode(),
		 * this.appConstants.getErrorRes().getMessage(),
